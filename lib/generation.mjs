...
export function openaiSize(model,width,height){
  if(!/^gpt-image-(?:2(?:-\d{4}-\d{2}-\d{2})?|2\.5-(?:sunburst|flare)(?:-\d{4}-\d{2}-\d{2})?)$/.test(model))return width===height?'1024x1024':width>height?'1536x1024':'1024x1536';
  const ratio=Math.max(1/3,Math.min(3,width/height));
  let best,score=Infinity;
  // Keep approximately one megapixel while respecting 16px increments and API bounds.
  for(let w=16;w<=3840;w+=16){
    for(const h of new Set([Math.floor(w/ratio/16)*16,Math.ceil(w/ratio/16)*16])){
      const area=w*h;if(h<16||h>3840||area<655360||area>1572864||w/h<1/3||w/h>3)continue;
      const candidate=Math.abs(Math.log((w/h)/ratio))*100+Math.abs(Math.log(area/1048576))*.01;
      if(candidate<score){score=candidate;best=`${w}x${h}`;}
    }
  }
  return best;
}
export function providerRequest({provider, apiKey, model, prompt, width, height}) {
  ...
  return {
    ...
    body:provider === 'openai' ? {model,prompt:`${prompt}\nComponha o criativo diretamente em toda a área da imagem de saída. Estenda o fundo até as bordas; não coloque o anúncio dentro de uma folha, moldura externa ou mockup. Preserve todos os textos e botões dentro da composição.`,n:1,size:openaiSize(model,width,height),quality:'medium',output_format:'jpeg'} : {model,prompt,n:1,response_format:'b64_json',aspect_ratio:ratio}
  };
}
