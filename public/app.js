import {providers,formats,presets,composePrompt,imagePlacement} from './core.js';
import {requireAccess,secureFetch} from './access.js';
import {mountAgents,initCopies} from './copy.js';
mountAgents();
...
function page(name){document.querySelectorAll('.page').forEach(el=>el.hidden=el.id!==`page-${name}`);document.querySelectorAll('.nav-item').forEach(el=>el.classList.toggle('active',el.dataset.page===name));$('breadcrumb').textContent={home:'Central de agentes',copy:'Gerador de copys','copy-library':'Prompts de texto','copy-history':'Histórico de textos',generator:'Gerar criativos',library:'Prompts de imagem',gallery:'Minhas imagens',settings:'Configurações'}[name];window.scrollTo({top:0,behavior:'smooth'});}
...
function renderKeyStatus(){Object.keys(providers).forEach(id=>{const saved=remote&&configuredKeys[id];$('key-status-'+id).textContent=saved?'✓ Chave salva no servidor. Não é exibida por segurança.':'Nenhuma chave salva'+(remote?' no servidor.':' nesta aba.');$('key-'+id).placeholder=saved?'Chave salva • cole outra para substituir':'Cole sua chave aqui';});}
renderKeyStatus();
$('settings-form').onsubmit=async e=>{e.preventDefault();const nextModels={},newKeys={};Object.keys(providers).forEach(id=>{const value=$('key-'+id).value.trim();if(value)newKeys[id]=value;nextModels[id]=$('model-'+id).value.trim();});try{if(remote){await api('settings','PUT',{models:nextModels,keys:newKeys});const confirmed=await api('settings');configuredKeys=confirmed.configuredKeys;Object.keys(newKeys).forEach(id=>{if(configuredKeys[id])$('key-'+id).value='';});renderKeyStatus();}else{keys=newKeys;await saveLocal('adstudio-models',nextModels);}models=nextModels;renderProviders();$('settings-message').textContent=remote?'Configurações salvas no servidor.':'Configurações aplicadas a esta aba.';toast('Configurações salvas.');}catch(error){toast(error.message);}};
...
function drawCrop(){if(!cropImage)return;const size=$('crop-format').value==='original'?cropItem:formats[Number($('crop-format').value)];const {w,h}=size;const canvas=$('crop-canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);const r=imagePlacement(cropImage.naturalWidth,cropImage.naturalHeight,w,h,Number($('crop-zoom').value),Number($('crop-x').value),Number($('crop-y').value));ctx.imageSmoothingQuality='high';ctx.drawImage(cropImage,r.dx,r.dy,r.dw,r.dh);$('zoom-value').textContent=Math.round(Number($('crop-zoom').value)*100)+'%';$('quality-value').textContent=`${Math.round(Number($('export-quality').value)*100)}%`;$('export-quality').disabled=$('export-type').value==='image/png';exportBlob=null;$('download-crop').disabled=true;const version=++exportVersion;canvas.toBlob(blob=>{if(version!==exportVersion)return;exportBlob=blob;$('download-crop').disabled=!blob;$('export-info').textContent=blob?`${w} × ${h} px · ${(blob.size/1024).toFixed(0)} KB${blob.size>150*1024?' · Verifique o limite de peso do posicionamento.':''}`:'Não foi possível preparar o download.';},$('export-type').value,Number($('export-quality').value));}
['crop-format','crop-zoom','crop-x','crop-y','export-type','export-quality'].forEach(id=>$(id).addEventListener('input',drawCrop));$('reset-crop').onclick=resetCrop;$('fit-crop').onclick=()=>{if(!cropImage)return;const {w,h}=$('crop-format').value==='original'?cropItem:formats[Number($('crop-format').value)];const {fitZoom}=imagePlacement(cropImage.naturalWidth,cropImage.naturalHeight,w,h);$('crop-zoom').value=Math.max(.001,Math.floor(fitZoom*1000)/1000);$('crop-x').value=.5;$('crop-y').value=.5;drawCrop();};
...
page('home');
void initCopies({api,toast,page}).catch(error=>toast(error.message));
