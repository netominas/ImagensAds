...
let models=read('adstudio-models',{}),keys={},provider='gemini',count=1,images=[],currentResults=[],busy=false,editingId=null,cropImage=null,cropItem=null,exportBlob=null,exportVersion=0;
...
function saveLocal(key,value){try{localStorage.setItem(key,JSON.stringify(value));return true;}catch{toast('Não foi possível salvar neste navegador. Verifique o espaço disponível.');return false;}}
...
function renderProviders(){
  ...
  $('provider-hint').textContent=keys[provider]?'✓ Chave configurada nesta aba':'◈ Configure sua chave de API para começar.';
}
renderProviders();
...
$('settings-form').onsubmit=e=>{e.preventDefault();Object.keys(providers).forEach(id=>{keys[id]=$(`key-${id}`).value.trim();models[id]=$(`model-${id}`).value.trim();});saveLocal('adstudio-models',models);renderProviders();$('settings-message').textContent='Configurações aplicadas a esta aba.';toast('Configurações salvas. Você já pode criar.');};
function renderTemplates(){
  ...
  $('prompt-library').querySelectorAll('[data-delete]').forEach(el=>el.onclick=()=>{if(templates.length===1)return toast('Mantenha pelo menos um prompt na biblioteca.');templates=templates.filter(p=>p.id!==el.dataset.delete);saveLocal('adstudio-prompts',templates);renderTemplates();});
  updatePrompt();
}
...
const dbPromise=new Promise((resolve,reject)=>{const req=indexedDB.open('adstudio-images',1);req.onupgradeneeded=()=>req.result.createObjectStore('images',{keyPath:'id'});req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});
...
async function addImage(item){try{await dbAction('readwrite',s=>s.put(item));}catch{toast('Imagem disponível nesta sessão, mas sem espaço para salvar no navegador. Baixe agora.');}const hydrated=hydrate(item);images.unshift(hydrated);renderImages();return hydrated;}
async function loadImages(){try{images=(await dbAction('readonly',s=>s.getAll())).sort((a,b)=>b.created-a.created).map(hydrate);renderImages();}catch{toast('O navegador não permitiu abrir a galeria local.');}}await loadImages();
function card(item){return `<article class="image-card"><img src="${item.url}" alt="${escapeHTML(item.theme)}" loading="lazy"><div class="image-card-body"><strong>${escapeHTML(item.theme)}</strong><p>${escapeHTML(providers[item.provider]?.name||'Importada')} · ${item.w} × ${item.h} px · ${new Date(item.created).toLocaleDateString('pt-BR')}</p><div class="image-actions"><button class="button light" data-crop="${item.id}">⌗ Recortar e baixar</button><button class="icon-button" data-remove="${item.id}" aria-label="Excluir imagem">×</button></div></div></article>`;}
...
async function pollJob(job){
  ...
  while(Date.now()-job.created<14*60*1000){
    try {
      ...
      if(state.status==='pending'&&Date.now()-job.created>60000)status('A geração ainda não iniciou. Confira se as funções e o Netlify Blobs estão habilitados na hospedagem.',true);
...
$('generation-form').onsubmit=async e=>{
  ...
  try{
    ...
    const res=await fetch('/.netlify/functions/generate-background',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:job.token,provider,model:models[provider]||providers[provider].model,apiKey:keys[provider],prompt:$('prompt-preview').textContent,width:w,height:h,count}),signal:AbortSignal.timeout(20000)});
